...
$(TEST_BINS): $(TEST_BIN_DIR)/%: $(TEST_DIR)/%.cc $(SRC_DIR)/calculator.cc $(SRC_DIR)/logger.cc $(SRC_DIR)/error_handler.cc
	@mkdir -p $(TEST_BIN_DIR)
	$(CC) $(CFLAGS) $(GTEST_FLAGS) -o $@ $< $(filter-out $(SRC_DIR)/main.cc, $(SOURCES))

$(CPPCHECK_XML): $(SOURCES) $(HEADERS)
	@mkdir -p $(ANALYSIS_DIR)
	cppcheck --enable=all --std=c++03 --xml -I $(INCLUDE_DIR) $(SRC_DIR) 2> $(CPPCHECK_XML)

analysis: $(CPPCHECK_XML)
	@mkdir -p $(HTML_REPORT_DIR)
	cppcheck-htmlreport --file=$(CPPCHECK_XML) --report-dir=$(HTML_REPORT_DIR) --source-dir=$(SRC_DIR)
